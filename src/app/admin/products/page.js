    ...
    <section className="flex-1 p-4 md:p-6 lg:p-8">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow p-4 md:p-6 mb-6">
            ...
            <div className="mt-6">
              <button onClick={save} disabled={loading || imageUploading} className={`w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[var(--brand)] text-white px-6 py-2 rounded text-sm ${loading || imageUploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                {loading ? 'Saving...' : imageUploading ? 'Uploading Image...' : (form.id ? 'Update Product' : 'Add Product')}
              </button>
              {form.id && (
                <button 
