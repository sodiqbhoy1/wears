...
export async function POST(req) {
  ...
  // Webhook signature verified
    ...
    if (body.event === 'charge.success' && body.data.status === 'success') {
      ...
      // Try Mongoose Order and Product models first
      let OrderModel = null;
      ...
      const query = { $or: [{ reference: body.data.reference }, { paystack_reference: body.data.reference }] };
      const existingOrder = OrderModel
        ? await OrderModel.findOne(query).lean().exec()
      ...
      if (order.items && order.items.length > 0) {
        for (const item of order.items) {
          try {
             ...
             console.error('Failed to deduct quantity in webhook:', err);
          }
        }
      }
