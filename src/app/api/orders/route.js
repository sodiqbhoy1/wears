...
export async function POST(req) {
  try {
    ...
    if (order.paid && order.items && order.items.length > 0) {
        ...
        try {
          ...
          if (Product) {
            await Product.updateOne(
              ...
              { $inc: { "variants.$.quantity": -item.qty } }
            );
            // Optionally triggering save to update status?
            // Since updateOne bypasses middleware, status won't update automatically.
            // We can fetch and save to trigger middleware if needed, but per-order fetch is expensive.
            // For now, let's keep it simple.
          } else {
            // Native driver
             await db.collection('products').updateOne(
                { 
                  _id: new ObjectId(item.id || item._id),
                  variants: { $elemMatch: { color: item.color, size: item.size } }
                },
                { $inc: { "variants.$.quantity": -item.qty } }
             );
